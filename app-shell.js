(function(){
  ...
  async function portalizeInfoPage(){
    const qs=new URLSearchParams(location.search);if(qs.get('portal')!=='1'||document.querySelector('.portal-nav'))return;
    if(!window.HCAuth?.auth)return;const u=await new Promise(res=>{const off=HCAuth.auth.onAuthStateChanged(x=>{off();res(x)})});if(!u){location.replace('executive-login.html?return='+encodeURIComponent(path+location.search));return}
    let p=HCAuth.getCachedProfile?.();try{if(!p)p=(await HCAuth.api('getProfile')).profile}catch(_){return}
    const old=document.querySelector('header nav');if(old){old.className='portal-nav';old.innerHTML=`<a href="${p.role==='admin'?'admin-dashboard.html':'executive-dashboard.html'}">Home</a><div class="nav-dropdown"><button type="button" class="nav-drop-btn">Loans ▾</button><div class="nav-dropdown-menu"><a href="personal-loan.html?portal=1">Personal Loan</a><a href="home-loan.html?portal=1">Home Loan</a><a href="home-loan-bt-topup.html?portal=1">Home Loan BT + Top-up</a><a href="business-loan.html?portal=1">Business Loan</a><a href="lap-loan.html?portal=1">Loan Against Property</a><a href="vehicle-loan.html?portal=1">Vehicle Loan</a><a href="education-loan.html?portal=1">Education Loan</a><a href="balance-transfer.html?portal=1">Balance Transfer / Consolidation</a><a href="project-funds.html?portal=1">Project Funds</a></div></div><div class="nav-dropdown"><button type="button" class="nav-drop-btn">Leads ▾</button><div class="nav-dropdown-menu"><a href="my-leads.html">My Leads</a><a data-feature="queue" href="lead-queue.html">Assigned Queue</a><a data-feature="addLead" href="new-lead.html">Add Lead</a></div></div><div class="nav-dropdown"><button type="button" class="nav-drop-btn">Resources ▾</button><div class="nav-dropdown-menu"><a data-feature="loanRates" href="loan-rates.html">Loan Rates</a><a data-feature="schemes" href="government-schemes.html">Schemes</a><a data-feature="mediaLibrary" href="media-library.html">Media Library</a></div></div><div class="nav-dropdown"><button type="button" class="nav-drop-btn">Marketing ▾</button><div class="nav-dropdown-menu"><a data-feature="socialPublisher" href="social-publisher.html">Social Publisher</a><a data-feature="campaignManager" href="campaign-manager.html">Campaigns</a><a data-feature="aiStudio" href="ai-marketing-studio.html">AI Marketing Studio</a><a data-feature="socialConnections" href="social-config.html">Social Connections</a></div></div><a data-feature="profile" href="executive-profile.html">Profile</a><button class="btn primary small" id="logout" type="button">Logout</button>`;old.querySelector('#logout').onclick=HCAuth.logout;const perm=p.permissions||{};if(p.role!=='admin')old.querySelectorAll('[data-feature]').forEach(a=>{if(perm[a.dataset.feature]!==true)a.hidden=true});old.querySelectorAll('.nav-dropdown').forEach(d=>{const b=d.querySelector('.nav-drop-btn');if(b)b.onclick=e=>{e.preventDefault();d.classList.toggle('open')}});document.body.classList.add('portal-body')}}
  function enhanceForms(){document.querySelectorAll('form').forEach(f=>{if(f.dataset.hcFormUi==='1')return;f.dataset.hcFormUi='1';f.classList.add('hc-form-modern');f.querySelectorAll('input:not([type=hidden]):not([type=checkbox]):not([type=radio]),select,textarea').forEach(el=>{const label=el.closest('label');if(label)label.classList.add('hc-field');el.classList.add('hc-control')});f.querySelectorAll('button[type=submit],input[type=submit]').forEach(b=>b.classList.add('hc-form-submit'))})}
  function historyBehavior(){window.addEventListener('pageshow',e=>{hide();document.body.classList.remove('hc-nav-busy');document.querySelectorAll('.nav-dropdown.open').forEach(x=>x.classList.remove('open'));if(e.persisted){enhanceForms();tooltips()}});window.addEventListener('popstate',()=>{hide();document.body.classList.remove('hc-nav-busy')})}
