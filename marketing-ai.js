(function(){
 ...
 function localGenerate(o={}){const loan=o.loanType||'Personal Loan',platform=o.platform||'Social Media',tone=o.tone||'Professional',aud=o.audience||'Customers',objective=o.objective||'Lead Generation',language=o.language||'English';const hs=hooks[loan]||hooks['Personal Loan'];const vi=Number(o.variationIndex||0);const h=hs[Math.abs((o.details||'').length+vi)%hs.length];const base=(templates.find(x=>x[0]===objective)||templates[0])[1];const cta=o.cta||'Know More';const details=(o.details||'').trim();const hashtags=['#Loans','#Finance','#LoanSupport','#HelloCustomer'];const openers=['Here is a simple way to start:','A better decision starts with the complete picture:','Before choosing an option, organise the requirement:','Make the next discussion more useful:'];const text=`${h}\n\n${openers[vi%openers.length]} ${base}${details?'\n\n'+details:''}\n\n${cta}: {{TRACKING_LINK}}\n\n${hashtags.join(' ')}`;const prompt=`Create a ${tone.toLowerCase()} ${platform} marketing post for ${loan}. Audience: ${audienceSafe(aud)}. Objective: ${objective}. Language: ${language}. Avoid guaranteed approval/rate claims. Include a clear CTA and use {{TRACKING_LINK}}.`;return{headline:h,text,prompt,hashtags:hashtags.join(' '),source:'Local marketing rules engine'}}
